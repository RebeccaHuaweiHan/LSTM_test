...
################################################################################
# Figure 4.23 The LIME values for PM10 with Lags from 0 to 24 in a LSTM model with Houston dataset.
################################################################################

LimeDF10Cities <- read.csv("~/deeplearning/LSTM_Test/LSTM_Test/data_nmmaps/LIME/US10Cities_epoch8000_seq5.csv")
# for city "hous"
# PM10LIMEvalue~ date
okabe_ito_palette <- c("Warm" = "#E69F00", "Cold" = "#56B4E9") # Orange & Blue, but distinguishable
...
ggplot(DF)+
  geom_line(aes(x= date, y= abs(LIMEvalue),color=WoC, linetype = WoC, group = interaction(WoC,year_season)))+
  ...
  scale_linetype_manual(values = c("Warm" = "solid", "Cold" = "dotted")) + # Use distinct line types
  scale_x_date(date_breaks = "2 years", date_labels = "%b %Y")+
  guides(color = guide_legend(title = "Season"), linetype = guide_legend(title = "Season")) + # Merge into one legend
  facet_grid(rows = vars(Lag)) +  
  facet_grid(rows=vars(Lag))+  
  theme(
    legend.position = "bottom"
    ) 
# ggsave(filename = "~/deeplearning/LSTM_Test/LSTM_Test/data_nmmaps/LIME/LIMEPM10~date.png",height = 7,    width = 6,dpi = 500)
################################################################################

################################################################################
# Figure 4.24 The LIME values for PM10 with Lags 0 in a LSTM model applied to the Houston dataset.
################################################################################
...
ggplot(DF) +
  ...
  scale_color_manual(
    values = c("Warm" = "#E69F00", "Cold" = "#56B4E9") 
  )+
  scale_x_date(date_breaks = "4 months", date_labels = "%b %Y")+
  theme(legend.position = "bottom")
# ggsave(filename = "~/deeplearning/LSTM_Test/LSTM_Test/data_nmmaps/LIME/LIMEPM10~dateZoomin_color.png",height = 4,    width = 6,dpi = 500)
#################################################################################
...
################################################################################
# Figure 4.25 The LIME values for PM10 VS. daily temperatures.
################################################################################
LimeDF10Cities <- read.csv("~/deeplearning/LSTM_Test/LSTM_Test/data_nmmaps/LIME/US10Cities_epoch8000_seq5.csv")
library(scales)
...
################################################################################
# Figure 4.26 The LIME values for PM10 VS. daily temperatures with San Diego dataset.
################################################################################
...
```{r}
################################################################################
# Figure 4.22 The LIME values for PM10 with Lags from 0 to 4 in a LSTM model with the Chicago dataset
################################################################################
...
origDF <- readxl::read_xlsx(origfilepath,.name_repair = "unique_quiet")
origDF <- origDF[,-c(1,2)]
origDF$date <- as.character(origDF$date)
origDF$date <- as.Date(origDF$date,format="%Y-%m-%d")
origDF$date <- as.character(origDF$date)
...
```{r}
################################################################################
# Figure 4.1 to 4.8 The LIME values for 4 air pollutions in the LSTM model with the Chicago dataset.
################################################################################
...
#################################################################################
# Figure 4.1 The LIME explanation at two local instances in the LSTM model with the Chicago dataset.
#################################################################################
...
plot1 <- ggplot(DF, aes(x = Lag, y = LIMEvalue,color=predictor,fill=predictor)) +
  ...
  scale_fill_viridis_d(name = "air pollutants") +  # Applying a color-blind friendly palette
...
plot2 <- ggplot(DF, aes(x = Lag, y = LIMEvalue,color=predictor,fill=predictor)) +
  geom_bar(stat = "identity",position = "dodge",color="lightgrey") +
  scale_fill_viridis_d() +  # Applying a color-blind friendly palette
  scale_color_viridis_d() +
  coord_flip() +
...
combplot
# ggsave(filename = "~/deeplearning/LSTM_Test/LSTM_Test/data_nmmaps/LIME/LIME_instance_colorfriendly.png",plot=combplot,dpi = 500)



################################################################################
# Figure 4.2 The LIME values for 4 air pollutions with Lag 0 in the LSTM model with the Chicago dataset.
################################################################################
...
################################################################################
# Figure 4.3 The LIME values for 4 air pollutions with Lag 1 in the LSTM model with the Chicago dataset.
################################################################################
...
################################################################################
# Figure 4.4 The LIME values for 4 air pollutions with Lag 2 in the LSTM model with the Chicago dataset.
################################################################################
...
################################################################################
# Figure 4.5 The LIME values for 4 air pollutions with Lag 3 in the LSTM model with the Chicago dataset.
################################################################################
...
################################################################################
# Figure 4.6 The LIME values for 4 air pollutions with Lag 4 in the LSTM model with the Chicago dataset.
################################################################################
...
#################################################################################
# Figure 4.7 The monthly LIME values for 4 air pollutions with Lag 0 in the LSTM model with the Chicago dataset.
#################################################################################
...
########################################################################################
# Figure 4.8 The Yearly Average LIME values for O3 in the LSTM model with the Chicago dataset.
########################################################################################

# Define a color-blind friendly palette (Okabe-Ito)
okabe_ito_palette <- c("#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2")
DF %>% ggplot() +
  geom_line(aes(x = Year_floor, y = Year_avg_LIMEvalue, color = Lag, linetype = Lag)) +
  geom_point(aes(x = Year_floor, y = Year_avg_LIMEvalue, color = Lag, shape = Lag), size = 2) +
  geom_smooth(method = "loess", se = TRUE, span = 0.5, 
              aes(x = YMonth, y = LIMEvalue, group = Lag), 
              color = NA,fullrange = FALSE, level = 0.95) +
  ...
  scale_shape_manual(values = c(16, 17, 18, 15, 3)) + # Different point shapes
  xlab("Time") +
  ...
  guides(color = guide_legend(ncol = 3), linetype = guide_legend(ncol = 3), shape = guide_legend(ncol = 3)) 
